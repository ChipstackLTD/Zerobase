...
void setup() {
  Serial1.begin(9600);  // Khởi tạo giao tiếp Serial1 ở tốc độ 9600 baud để in dữ liệu.
  while (!Serial1) {
    ;  // Đợi cổng Serial1 kết nối. Cần thiết cho các bo mạch có cổng USB gốc.
  }
  ...
  Serial1.print("Map value: ");  // In ra chuỗi "Map value: "
  Serial1.println(val);          // In giá trị 'val' đã được ánh xạ ra Serial Monitor (kết quả sẽ là 255).
}
